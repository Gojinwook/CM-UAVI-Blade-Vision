...
/////////////////////////////////////////////////////////////////////////////
// CInspectViewExtra1Dlg dialog

CInspectViewExtra1Dlg* CInspectViewExtra1Dlg::m_pInstance = NULL;

CInspectViewExtra1Dlg* CInspectViewExtra1Dlg::GetInstance(BOOL bShowFlag)
{
	if (!m_pInstance) {
		m_pInstance = new CInspectViewExtra1Dlg();
		if(!m_pInstance->m_hWnd) {
			CMainFrame* pFrame = (CMainFrame*)AfxGetMainWnd();
			m_pInstance->Create(IDD_INSPECT_VIEW_EXTRA1_DLG, pFrame->GetActiveView());
...
void CInspectViewExtra1Dlg::DeleteInstance()
{
...
void CInspectViewExtra1Dlg::Show()
{
	ShowWindow(SW_HIDE);
...
CInspectViewExtra1Dlg::CInspectViewExtra1Dlg(CWnd* pParent /*=NULL*/)
	: CDialog(CInspectViewExtra1Dlg::IDD, pParent)
{
	//{{AFX_DATA_INIT(CInspectViewExtra1Dlg)
		// NOTE: the ClassWizard will add member initialization here
	//}}AFX_DATA_INIT
	SetPosition(VIEW1_DLG1_LEFT+(VIEW1_DLG1_WIDTH+270), VIEW1_DLG1_TOP, (VIEW1_DLG1_WIDTH+240)/2, (VIEW1_DLG1_HEIGHT+370)/2);
...
void CInspectViewExtra1Dlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CInspectViewExtra1Dlg)
	DDX_Control(pDX, IDC_VSCRBAR, m_VScrBar);
	DDX_Control(pDX, IDC_HSCRBAR, m_HScrBar);
	//}}AFX_DATA_MAP
}

BEGIN_MESSAGE_MAP(CInspectViewExtra1Dlg, CDialog)
	//{{AFX_MSG_MAP(CInspectViewExtra1Dlg)
	ON_WM_ERASEBKGND()
	ON_WM_PAINT()
	ON_WM_HSCROLL()
	ON_WM_VSCROLL()
...
/////////////////////////////////////////////////////////////////////////////
// CInspectViewExtra1Dlg message handlers

void CInspectViewExtra1Dlg::ClearAll()
{
	m_dZoomRatio = 0.4;
...
LRESULT CInspectViewExtra1Dlg::OnEventPcbModelChanaged(WPARAM wParam, LPARAM lParam)
{
	Reset();
...
// For Release
LRESULT CInspectViewExtra1Dlg::OnEventFrameInspectDone(WPARAM wParam, LPARAM lParam)
{
...
BOOL CInspectViewExtra1Dlg::PreTranslateMessage(MSG* pMsg) 
{
	switch (pMsg->message) {
...
int CInspectViewExtra1Dlg::Get_CurScanTab()
{
	return m_ViewToolbar.Get_CurComboBox();
}

Hlong CInspectViewExtra1Dlg::GetDispWindowID()
{
	return m_lWindowID;
}

void CInspectViewExtra1Dlg::Set_CurScanTab(int nScanNum)
{ 
	CString strScan;
...
void CInspectViewExtra1Dlg::Add_ScanTab(int nScanNum)
{
	CString strScan;
	strScan.Format("SCAN_%d", nScanNum);
...
void CInspectViewExtra1Dlg::Clear_ScanTab()
{
	m_ViewToolbar.Clear_ComboBox();
}

int CInspectViewExtra1Dlg::Get_ScanTabCount()
{
	return m_ViewToolbar.Get_ComboBoxCount();
}

int CInspectViewExtra1Dlg::Get_ScanTabIndex()
{
	return m_ViewToolbar.Get_ComboBoxIndex();
}

void CInspectViewExtra1Dlg::Set_ScanTabIndex(int nIdx)
{
	m_ViewToolbar.Set_ComboBoxIndex(nIdx);
}

BOOL CInspectViewExtra1Dlg::OnInitDialog() 
{
	CDialog::OnInitDialog();

	SetWindowText(g_sImageName[THEAPP.m_iMachineInspType][TRIGGER_EXTRA1]);
...
BOOL CInspectViewExtra1Dlg::OnEraseBkgnd(CDC *pDC)
{
 	pDC->FillSolidRect(m_ClientRect, TS_COLOR_EXTRA_BG);
	return TRUE;
}

void CInspectViewExtra1Dlg::OnPaint() 
{
...
void CInspectViewExtra1Dlg::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) 
{
...
void CInspectViewExtra1Dlg::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) 
{
	if (m_lWindowID < 0 || !m_bDisplayImage) return;
...
void CInspectViewExtra1Dlg::OnLButtonDown(UINT nFlags, CPoint point) 
{
...
void CInspectViewExtra1Dlg::OnLButtonUp(UINT nFlags, CPoint point) 
{
	if (!m_bDisplayImage) return;
...
void CInspectViewExtra1Dlg::OnMouseMove(UINT nFlags, CPoint point) 
{
	if (!m_bDisplayImage) return;
	...
	CString OutTxt;
	OutTxt.Format("%s - (%d, %d), Zoom(%d%%)", g_sImageName[THEAPP.m_iMachineInspType][TRIGGER_EXTRA1], (int)((point.x/m_dZoomRatio)+m_ViewRect.left), (int)((point.y/m_dZoomRatio)+m_ViewRect.top), (int)(m_dZoomRatio*100.));
...
void CInspectViewExtra1Dlg::OnDestroy() 
{
	CDialog::OnDestroy();
...
void CInspectViewExtra1Dlg::OnSize(UINT nType, int cx, int cy) 
{
...
BOOL CInspectViewExtra1Dlg::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt) 
{
...
BOOL CInspectViewExtra1Dlg::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message) 
{
	switch (m_ToolBarState) {
	case TS_MOVE:
		if (m_bOnMoving)
...
void CInspectViewExtra1Dlg::Reset()
{
...
void CInspectViewExtra1Dlg::InitViewRect() 
{
	m_ViewRect.left = 0;
	m_ViewRect.top = 0;
...
void CInspectViewExtra1Dlg::UpdateViewportManager()
{
...
void CInspectViewExtra1Dlg::ViewRectSet() 
{
...
void CInspectViewExtra1Dlg::ScrollBarSet()
{
	SCROLLINFO Info;
...
void CInspectViewExtra1Dlg::ScrollBarPosSet()
{
...
void CInspectViewExtra1Dlg::DrawActiveTRegion()
{
...
void CInspectViewExtra1Dlg::DrawSelectPartTRegion()
{
	CDC *pDC = GetDC();
...
void CInspectViewExtra1Dlg::ZoomInSelectedPart(double dZoomRatio, int iStartPosX, int iStartPosY) 
{
	if (m_bDisplayImage) {
...
//		m_ViewportManager.mdZoomRatio = m_dZoomRatio;
		...
		CString OutTxt;
		OutTxt.Format("%s - Zoom(%d%%)", g_sImageName[THEAPP.m_iMachineInspType][TRIGGER_EXTRA1], (int)(m_dZoomRatio * 100.));
...
void CInspectViewExtra1Dlg::OnTbDraw() 
{
	m_ToolBarState = TS_DRAW;
}

void CInspectViewExtra1Dlg::OnTbMove() 
{
 	m_ToolBarState = TS_MOVE;
}

void CInspectViewExtra1Dlg::OnTbSelectPart() 
{
 	m_ToolBarState = TS_SELECT_PART;
}

void CInspectViewExtra1Dlg::OnTbZoomIn() 
{
	if (m_bDisplayImage) {
...
void CInspectViewExtra1Dlg::OnTbZoomOut() 
{
	if (m_bDisplayImage) {
		m_dZoomRatio = (int)(m_dZoomRatio * 10) * 0.1;
...
void CInspectViewExtra1Dlg::OnTbFitWidth() 
{
	if (m_bDisplayImage) {
...
void CInspectViewExtra1Dlg::OnTbFitHeight() 
{
	if (m_bDisplayImage) {
...
BOOL CInspectViewExtra1Dlg::ShowWindow(int nCmdShow)
{
	return CDialog::ShowWindow(nCmdShow);
}

void CInspectViewExtra1Dlg::InitializeToolBar()
{
	CToolBarCtrl &toolbarctrl = m_ViewToolbar.GetToolBarCtrl();
...
int CInspectViewExtra1Dlg::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
...
LRESULT CInspectViewExtra1Dlg::On2DFrameReady(WPARAM wParam, LPARAM lParam)
{

	return 0;
}

LRESULT CInspectViewExtra1Dlg::OnEventGoToDefect(WPARAM wParam, LPARAM lParam)	
{
...
void CInspectViewExtra1Dlg::LoadScanImage(int nScanNum) 
{

}

void CInspectViewExtra1Dlg::MoveToDefectArea()
{
	
}

void CInspectViewExtra1Dlg::RedrawView()
{
	InvalidateRect(NULL, FALSE);
}

void CInspectViewExtra1Dlg::RedrawRect()
{
	InvalidateRect(&m_ClientRect, FALSE);
}

BOOL CInspectViewExtra1Dlg::SelectAllPackage()
{
	return FALSE;
}

BOOL CInspectViewExtra1Dlg::SelectPackageChange() 
{
	return FALSE;
}

BOOL CInspectViewExtra1Dlg::SelectPackageUndo() 
{
	return FALSE;
}

void CInspectViewExtra1Dlg::UnSelectAll()
{
