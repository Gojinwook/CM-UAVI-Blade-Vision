// TrayOperatorViewNowDlg.cpp : 구현 파일입니다.
//
...
CTrayOperatorViewNowDlg* CTrayOperatorViewNowDlg::m_pInstance = NULL;

CTrayOperatorViewNowDlg* CTrayOperatorViewNowDlg::GetInstance(BOOL bShowFlag)
{
	if (!m_pInstance) {
		m_pInstance = new CTrayOperatorViewNowDlg();
		if(!m_pInstance->m_hWnd) {
			CMainFrame* pFrame = (CMainFrame*)AfxGetMainWnd();
			m_pInstance->Create(IDD_TRAYVIEW_OPERATOR_CURRENT_DLG, pFrame->GetActiveView());
			if (bShowFlag) m_pInstance->Show();
		}
	}
	return m_pInstance;
}
void CTrayOperatorViewNowDlg::DeleteInstance()
{
...
void CTrayOperatorViewNowDlg::Show()
{
	ShowWindow(SW_HIDE);
...
IMPLEMENT_DYNAMIC(CTrayOperatorViewNowDlg, CDialog)

CTrayOperatorViewNowDlg::CTrayOperatorViewNowDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CTrayOperatorViewNowDlg::IDD, pParent)
{
	SetPosition(VIEW1_DLG1_LEFT+VIEW1_DLG1_WIDTH+15, VIEW1_DLG1_TOP, VIEW1_DLG1_WIDTH+25, VIEW1_DLG1_HEIGHT+380);
}

CTrayOperatorViewNowDlg::~CTrayOperatorViewNowDlg()
{
}

void CTrayOperatorViewNowDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CTrayOperatorViewNowDlg, CDialog)
	ON_WM_PAINT()
	ON_WM_CTLCOLOR()
END_MESSAGE_MAP()


// CTrayOperatorViewNowDlg 메시지 처리기입니다.
void CTrayOperatorViewNowDlg::UpdateTrayDisplay()
{

}

void CTrayOperatorViewNowDlg::OnPaint()
{
...
BOOL CTrayOperatorViewNowDlg::PreTranslateMessage(MSG* pMsg)
{
...
HBRUSH CTrayOperatorViewNowDlg::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
CFont* CTrayOperatorViewNowDlg::GetFont(LONG lfWidth, LONG lfHeight, LONG lfWeight)
 {
        CFont* pFont;
        LOGFONT logFont;
...
void CTrayOperatorViewNowDlg::MakeTrayRectangle()
{
