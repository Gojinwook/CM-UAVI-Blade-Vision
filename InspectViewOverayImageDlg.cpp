...
/////////////////////////////////////////////////////////////////////////////
// CInspectViewOverayImageDlg dialog

CInspectViewOverayImageDlg* CInspectViewOverayImageDlg::m_pInstance = NULL;

CInspectViewOverayImageDlg* CInspectViewOverayImageDlg::GetInstance(BOOL bShowFlag)
{
	if (!m_pInstance) {
		m_pInstance = new CInspectViewOverayImageDlg();
		if(!m_pInstance->m_hWnd) {
			CMainFrame* pFrame = (CMainFrame*)AfxGetMainWnd();
			m_pInstance->Create(IDD_INSPECT_VIEW_OVERAY_IMAGE, pFrame->GetActiveView());
...
void CInspectViewOverayImageDlg::DeleteInstance()
{
...
void CInspectViewOverayImageDlg::Show()
{
	ShowWindow(SW_HIDE);
...
CInspectViewOverayImageDlg::CInspectViewOverayImageDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CInspectViewOverayImageDlg::IDD, pParent)
{
	//{{AFX_DATA_INIT(CInspectViewOverayImageDlg)
		// NOTE: the ClassWizard will add member initialization here
	//}}AFX_DATA_INIT
	SetPosition(VIEW1_DLG3_LEFT+515, VIEW1_DLG3_TOP+40, (VIEW1_DLG1_WIDTH+320)/2, (VIEW1_DLG1_HEIGHT+430)/2);
...
void CInspectViewOverayImageDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CInspectViewOverayImageDlg)
	DDX_Control(pDX, IDC_VSCRBAR, m_VScrBar);
	DDX_Control(pDX, IDC_HSCRBAR, m_HScrBar);
	//}}AFX_DATA_MAP
}

BEGIN_MESSAGE_MAP(CInspectViewOverayImageDlg, CDialog)
	//{{AFX_MSG_MAP(CInspectViewOverayImageDlg)
	ON_WM_ERASEBKGND()
	ON_WM_PAINT()
	ON_WM_HSCROLL()
	ON_WM_VSCROLL()
...
/////////////////////////////////////////////////////////////////////////////
// CInspectViewOverayImageDlg message handlers

void CInspectViewOverayImageDlg::ClearAll()
{
	m_dZoomRatio = 0.4;
...
LRESULT CInspectViewOverayImageDlg::OnEventPcbModelChanaged(WPARAM wParam, LPARAM lParam)
{
	Reset();
...
// For Release
LRESULT CInspectViewOverayImageDlg::OnEventFrameInspectDone(WPARAM wParam, LPARAM lParam)
{
...
BOOL CInspectViewOverayImageDlg::PreTranslateMessage(MSG* pMsg) 
{
	switch (pMsg->message) {
...
int CInspectViewOverayImageDlg::Get_CurScanTab()
{
	return m_ViewToolbar.Get_CurComboBox();
}

void CInspectViewOverayImageDlg::Set_CurScanTab(int nScanNum)
{ 
	CString strScan;
...
void CInspectViewOverayImageDlg::Add_ScanTab(int nScanNum)
{
	CString strScan;
	strScan.Format("SCAN_%d", nScanNum);
...
void CInspectViewOverayImageDlg::Clear_ScanTab()
{
	m_ViewToolbar.Clear_ComboBox();
}

int CInspectViewOverayImageDlg::Get_ScanTabCount()
{
	return m_ViewToolbar.Get_ComboBoxCount();
}

int CInspectViewOverayImageDlg::Get_ScanTabIndex()
{
	return m_ViewToolbar.Get_ComboBoxIndex();
}

void CInspectViewOverayImageDlg::Set_ScanTabIndex(int nIdx)
{
	m_ViewToolbar.Set_ComboBoxIndex(nIdx);
}

BOOL CInspectViewOverayImageDlg::OnInitDialog() 
{
...
BOOL CInspectViewOverayImageDlg::OnEraseBkgnd(CDC *pDC)
{
 	pDC->FillSolidRect(m_ClientRect, TS_COLOR_EXTRA_BG);
	return TRUE;
}

void CInspectViewOverayImageDlg::OnPaint() 
{
...
void CInspectViewOverayImageDlg::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) 
{
...
void CInspectViewOverayImageDlg::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) 
{
	if (m_lWindowID < 0 || !m_bDisplayImage) return;
...
void CInspectViewOverayImageDlg::OnLButtonDown(UINT nFlags, CPoint point) 
{
...
void CInspectViewOverayImageDlg::OnLButtonUp(UINT nFlags, CPoint point) 
{
	if (!m_bDisplayImage) return;
...
void CInspectViewOverayImageDlg::OnMouseMove(UINT nFlags, CPoint point) 
{
	if (!m_bDisplayImage) return;
	...
	CString OutTxt;
	OutTxt.Format("Result - (%d, %d), Zoom(%d%%)", (int)((point.x/m_dZoomRatio)+m_ViewRect.left), (int)((point.y/m_dZoomRatio)+m_ViewRect.top), (int)(m_dZoomRatio*100.));
...
void CInspectViewOverayImageDlg::OnDestroy() 
{
	CDialog::OnDestroy();
...
void CInspectViewOverayImageDlg::OnSize(UINT nType, int cx, int cy) 
{
...
BOOL CInspectViewOverayImageDlg::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt) 
{
...
BOOL CInspectViewOverayImageDlg::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message) 
{
	switch (m_ToolBarState) {
	case TS_MOVE:
		if (m_bOnMoving)
...
void CInspectViewOverayImageDlg::Reset()
{
...
void CInspectViewOverayImageDlg::InitViewRect() 
{
	m_ViewRect.left = 0;
	m_ViewRect.top = 0;
...
void CInspectViewOverayImageDlg::UpdateViewportManager()
{
...
void CInspectViewOverayImageDlg::ViewRectSet() 
{
...
void CInspectViewOverayImageDlg::ScrollBarSet()
{
	SCROLLINFO Info;
...
void CInspectViewOverayImageDlg::ScrollBarPosSet()
{
...
void CInspectViewOverayImageDlg::DrawActiveTRegion()
{
...
void CInspectViewOverayImageDlg::DrawSelectPartTRegion()
{
	CDC *pDC = GetDC();
...
void CInspectViewOverayImageDlg::ZoomInSelectedPart(double dZoomRatio, int iStartPosX, int iStartPosY) 
{
	if (m_bDisplayImage) {
...
//		m_ViewportManager.mdZoomRatio = m_dZoomRatio;
		...
		CString OutTxt;
		OutTxt.Format("Result - Zoom(%d%%)", (int)(m_dZoomRatio * 100.));
...
void CInspectViewOverayImageDlg::OnTbDraw() 
{
	m_ToolBarState = TS_DRAW;
}

void CInspectViewOverayImageDlg::OnTbMove() 
{
 	m_ToolBarState = TS_MOVE;
}

void CInspectViewOverayImageDlg::OnTbSelectPart() 
{
 	m_ToolBarState = TS_SELECT_PART;
}

void CInspectViewOverayImageDlg::OnTbZoomIn() 
{
	if (m_bDisplayImage) {
...
void CInspectViewOverayImageDlg::OnTbZoomOut() 
{
	if (m_bDisplayImage) {
		m_dZoomRatio = (int)(m_dZoomRatio * 10) * 0.1;
...
void CInspectViewOverayImageDlg::OnTbFitWidth() 
{
	if (m_bDisplayImage) {
...
void CInspectViewOverayImageDlg::OnTbFitHeight() 
{
	if (m_bDisplayImage) {
...
BOOL CInspectViewOverayImageDlg::ShowWindow(int nCmdShow)
{
	return CDialog::ShowWindow(nCmdShow);
}

void CInspectViewOverayImageDlg::InitializeToolBar()
{
	CToolBarCtrl &toolbarctrl = m_ViewToolbar.GetToolBarCtrl();
...
int CInspectViewOverayImageDlg::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
...
LRESULT CInspectViewOverayImageDlg::On2DFrameReady(WPARAM wParam, LPARAM lParam)
{

	return 0;
}

LRESULT CInspectViewOverayImageDlg::OnEventGoToDefect(WPARAM wParam, LPARAM lParam)	
{
...
void CInspectViewOverayImageDlg::LoadScanImage(int nScanNum) 
{

}

void CInspectViewOverayImageDlg::MoveToDefectArea()
{
	
}

void CInspectViewOverayImageDlg::RedrawView()
{
	InvalidateRect(NULL, FALSE);
}

void CInspectViewOverayImageDlg::RedrawRect()
{
	InvalidateRect(&m_ClientRect, FALSE);
}

BOOL CInspectViewOverayImageDlg::SelectAllPackage()
{
	return FALSE;
}

BOOL CInspectViewOverayImageDlg::SelectPackageChange() 
{
	return FALSE;
}

BOOL CInspectViewOverayImageDlg::SelectPackageUndo() 
{
	return FALSE;
}

void CInspectViewOverayImageDlg::UnSelectAll()
{

}
void CInspectViewOverayImageDlg::CallFitImageHeight()
{
	OnTbFitHeight();
}