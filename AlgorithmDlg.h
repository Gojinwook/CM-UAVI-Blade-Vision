...
public:
	CAlgorithmDlg(CWnd* pParent = NULL);   // 표준 생성자입니다.
	virtual ~CAlgorithmDlg();

	static	CAlgorithmDlg* GetInstance(BOOL bShowFlag=FALSE);
	void	DeleteInstance();

	void	Show();
	void	Hide();

	void SetSelectedInspection(int iImageType, int iTabIndex);
	void SetParam();

// 대화 상자 데이터입니다.
	enum { IDD = IDD_TEACH_ALGORITHM_DLG };

private:
	void	SetPosition(int left, int top, int right, int bottom) { m_ScreenRect = CRect(left, top, right, bottom); }
	CRect	m_ScreenRect;

	static CAlgorithmDlg*	m_pInstance;
...
public:
	...
	int m_iEditDefectConditionEpoxyDist;
...
public:
	...
	int m_iEditEpoxySEThres;
	int m_iEditEpoxyESThres;
	int m_iEditEpoxyClosingSize;
	int m_iEditEpoxyOpeningSize;
	int m_iEditEpoxyNearDistance;
	int m_iEditEpoxyNearMinSize;
	BOOL m_bCheckEpoxyConditionHole;
	BOOL m_bCheckEpoxyConditionNear;
