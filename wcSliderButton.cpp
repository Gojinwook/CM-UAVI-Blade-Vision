...
// LRESULT wcSliderButton::OnSliderDelta(UINT lParam, LONG wParam)
LRESULT wcSliderButton::OnSliderDelta(WPARAM wParam, LPARAM lParam)
{
	UINT lParam2;
	LONG wParam2;
    ...
    if ( pParent ) 
		 pParent->SendMessage(CSP_DELTAPOS, lParam2, (WPARAM) GetDlgCtrlID());
		
	SetPos (lParam2);
    return TRUE;
}

// LRESULT wcSliderButton::OnSliderClose(UINT lParam, LONG wParam)
LRESULT wcSliderButton::OnSliderClose(WPARAM wParam, LPARAM lParam)
{
	UINT lParam2;
	LONG wParam2;
    ...
    CWnd *pParent = GetParent();
    if (pParent) {
        pParent->SendMessage(CSP_CLOSEUP, lParam2, (WPARAM) GetDlgCtrlID());
    }
