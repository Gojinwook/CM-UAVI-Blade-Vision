...
/////////////////////////////////////////////////////////////////////////////
// CInspectViewLensInnerDlg dialog

CInspectViewLensInnerDlg* CInspectViewLensInnerDlg::m_pInstance = NULL;

CInspectViewLensInnerDlg* CInspectViewLensInnerDlg::GetInstance(BOOL bShowFlag)
{
	if (!m_pInstance) {
		m_pInstance = new CInspectViewLensInnerDlg();
		if(!m_pInstance->m_hWnd) {
			CMainFrame* pFrame = (CMainFrame*)AfxGetMainWnd();
			m_pInstance->Create(IDD_INSPECT_VIEW_LENS_INNER_DLG, pFrame->GetActiveView());
...
void CInspectViewLensInnerDlg::DeleteInstance()
{
...
void CInspectViewLensInnerDlg::Show()
{
	ShowWindow(SW_HIDE);
...
CInspectViewLensInnerDlg::CInspectViewLensInnerDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CInspectViewLensInnerDlg::IDD, pParent)
{
	//{{AFX_DATA_INIT(CInspectViewLensInnerDlg)
		// NOTE: the ClassWizard will add member initialization here
	//}}AFX_DATA_INIT
	SetPosition(VIEW1_DLG1_LEFT, VIEW1_DLG1_TOP+(VIEW1_DLG1_HEIGHT+380)/2, (VIEW1_DLG1_WIDTH+290)/2, (VIEW1_DLG1_HEIGHT+380)/2);
...
void CInspectViewLensInnerDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CInspectViewLensInnerDlg)
	DDX_Control(pDX, IDC_VSCRBAR, m_VScrBar);
	DDX_Control(pDX, IDC_HSCRBAR, m_HScrBar);
	//}}AFX_DATA_MAP
}

BEGIN_MESSAGE_MAP(CInspectViewLensInnerDlg, CDialog)
	//{{AFX_MSG_MAP(CInspectViewLensInnerDlg)
	ON_WM_ERASEBKGND()
	ON_WM_PAINT()
	ON_WM_HSCROLL()
	ON_WM_VSCROLL()
...
/////////////////////////////////////////////////////////////////////////////
// CInspectViewLensInnerDlg message handlers

void CInspectViewLensInnerDlg::ClearAll()
{
	m_dZoomRatio = 0.4;
...
LRESULT CInspectViewLensInnerDlg::OnEventPcbModelChanaged(WPARAM wParam, LPARAM lParam)
{
	Reset();
...
// For Release
LRESULT CInspectViewLensInnerDlg::OnEventFrameInspectDone(WPARAM wParam, LPARAM lParam)
{
...
BOOL CInspectViewLensInnerDlg::PreTranslateMessage(MSG* pMsg) 
{
	switch (pMsg->message) {
...
int CInspectViewLensInnerDlg::Get_CurScanTab()
{
	return m_ViewToolbar.Get_CurComboBox();
}

void CInspectViewLensInnerDlg::Set_CurScanTab(int nScanNum)
{ 
	CString strScan;
...
void CInspectViewLensInnerDlg::Add_ScanTab(int nScanNum)
{
	CString strScan;
	strScan.Format("SCAN_%d", nScanNum);
...
void CInspectViewLensInnerDlg::Clear_ScanTab()
{
	m_ViewToolbar.Clear_ComboBox();
}

int CInspectViewLensInnerDlg::Get_ScanTabCount()
{
	return m_ViewToolbar.Get_ComboBoxCount();
}

int CInspectViewLensInnerDlg::Get_ScanTabIndex()
{
	return m_ViewToolbar.Get_ComboBoxIndex();
}

void CInspectViewLensInnerDlg::Set_ScanTabIndex(int nIdx)
{
	m_ViewToolbar.Set_ComboBoxIndex(nIdx);
}

BOOL CInspectViewLensInnerDlg::OnInitDialog() 
{
...
BOOL CInspectViewLensInnerDlg::OnEraseBkgnd(CDC *pDC)
{
 	pDC->FillSolidRect(m_ClientRect, TS_COLOR_EXTRA_BG);
	return TRUE;
}

void CInspectViewLensInnerDlg::OnPaint() 
{
...
void CInspectViewLensInnerDlg::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) 
{
...
void CInspectViewLensInnerDlg::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) 
{
	if (m_lWindowID < 0 || !m_bDisplayImage) return;
...
void CInspectViewLensInnerDlg::OnLButtonDown(UINT nFlags, CPoint point) 
{
...
void CInspectViewLensInnerDlg::OnLButtonUp(UINT nFlags, CPoint point) 
{
	if (!m_bDisplayImage) return;
...
void CInspectViewLensInnerDlg::OnMouseMove(UINT nFlags, CPoint point) 
{
	if (!m_bDisplayImage) return;
	...
	CString OutTxt;
	OutTxt.Format("Lens Inner - (%d, %d), Zoom(%d%%)", (int)((point.x/m_dZoomRatio)+m_ViewRect.left), (int)((point.y/m_dZoomRatio)+m_ViewRect.top), (int)(m_dZoomRatio*100.));
...
void CInspectViewLensInnerDlg::OnDestroy() 
{
	CDialog::OnDestroy();
...
void CInspectViewLensInnerDlg::OnSize(UINT nType, int cx, int cy) 
{
...
BOOL CInspectViewLensInnerDlg::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt) 
{
...
BOOL CInspectViewLensInnerDlg::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message) 
{
	switch (m_ToolBarState) {
	case TS_MOVE:
		if (m_bOnMoving)
...
void CInspectViewLensInnerDlg::Reset()
{
...
void CInspectViewLensInnerDlg::InitViewRect() 
{
	m_ViewRect.left = 0;
	m_ViewRect.top = 0;
...
void CInspectViewLensInnerDlg::UpdateViewportManager()
{
...
void CInspectViewLensInnerDlg::ViewRectSet() 
{
...
void CInspectViewLensInnerDlg::ScrollBarSet()
{
	SCROLLINFO Info;
...
void CInspectViewLensInnerDlg::ScrollBarPosSet()
{
...
void CInspectViewLensInnerDlg::DrawActiveTRegion()
{
...
void CInspectViewLensInnerDlg::DrawSelectPartTRegion()
{
	CDC *pDC = GetDC();
...
void CInspectViewLensInnerDlg::ZoomInSelectedPart(double dZoomRatio, int iStartPosX, int iStartPosY) 
{
	if (m_bDisplayImage) {
...
//		m_ViewportManager.mdZoomRatio = m_dZoomRatio;
		...
		CString OutTxt;
		OutTxt.Format("Lens Inner - Zoom(%d%%)", (int)(m_dZoomRatio * 100.));
...
void CInspectViewLensInnerDlg::OnTbDraw() 
{
	m_ToolBarState = TS_DRAW;
}

void CInspectViewLensInnerDlg::OnTbMove() 
{
 	m_ToolBarState = TS_MOVE;
}

void CInspectViewLensInnerDlg::OnTbSelectPart() 
{
 	m_ToolBarState = TS_SELECT_PART;
}

void CInspectViewLensInnerDlg::OnTbZoomIn() 
{
	if (m_bDisplayImage) {
...
void CInspectViewLensInnerDlg::OnTbZoomOut() 
{
	if (m_bDisplayImage) {
		m_dZoomRatio = (int)(m_dZoomRatio * 10) * 0.1;
...
void CInspectViewLensInnerDlg::OnTbFitWidth() 
{
	if (m_bDisplayImage) {
...
void CInspectViewLensInnerDlg::OnTbFitHeight() 
{
	if (m_bDisplayImage) {
...
BOOL CInspectViewLensInnerDlg::ShowWindow(int nCmdShow)
{
	return CDialog::ShowWindow(nCmdShow);
}

void CInspectViewLensInnerDlg::InitializeToolBar()
{
	CToolBarCtrl &toolbarctrl = m_ViewToolbar.GetToolBarCtrl();
...
int CInspectViewLensInnerDlg::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
...
LRESULT CInspectViewLensInnerDlg::On2DFrameReady(WPARAM wParam, LPARAM lParam)
{

	return 0;
}

LRESULT CInspectViewLensInnerDlg::OnEventGoToDefect(WPARAM wParam, LPARAM lParam)	
{
...
void CInspectViewLensInnerDlg::LoadScanImage(int nScanNum) 
{

}

void CInspectViewLensInnerDlg::MoveToDefectArea()
{
	
}

void CInspectViewLensInnerDlg::RedrawView()
{
	InvalidateRect(NULL, FALSE);
}

void CInspectViewLensInnerDlg::RedrawRect()
{
	InvalidateRect(&m_ClientRect, FALSE);
}

BOOL CInspectViewLensInnerDlg::SelectAllPackage()
{
	return FALSE;
}

BOOL CInspectViewLensInnerDlg::SelectPackageChange() 
{
	return FALSE;
}

BOOL CInspectViewLensInnerDlg::SelectPackageUndo() 
{
	return FALSE;
}

void CInspectViewLensInnerDlg::UnSelectAll()
{
