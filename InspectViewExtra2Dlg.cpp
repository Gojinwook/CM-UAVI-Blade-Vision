...
/////////////////////////////////////////////////////////////////////////////
// CInspectViewExtra2Dlg dialog

CInspectViewExtra2Dlg* CInspectViewExtra2Dlg::m_pInstance = NULL;

CInspectViewExtra2Dlg* CInspectViewExtra2Dlg::GetInstance(BOOL bShowFlag)
{
	if (!m_pInstance) {
		m_pInstance = new CInspectViewExtra2Dlg();
		if(!m_pInstance->m_hWnd) {
			CMainFrame* pFrame = (CMainFrame*)AfxGetMainWnd();
			m_pInstance->Create(IDD_INSPECT_VIEW_EXTRA2_DLG, pFrame->GetActiveView());
...
void CInspectViewExtra2Dlg::DeleteInstance()
{
...
void CInspectViewExtra2Dlg::Show()
{
	ShowWindow(SW_HIDE);
...
CInspectViewExtra2Dlg::CInspectViewExtra2Dlg(CWnd* pParent /*=NULL*/)
	: CDialog(CInspectViewExtra2Dlg::IDD, pParent)
{
	//{{AFX_DATA_INIT(CInspectViewExtra2Dlg)
		// NOTE: the ClassWizard will add member initialization here
	//}}AFX_DATA_INIT
	SetPosition(VIEW1_DLG1_LEFT+(VIEW1_DLG1_WIDTH+270), VIEW1_DLG1_TOP+(VIEW1_DLG1_HEIGHT+380)/2, (VIEW1_DLG1_WIDTH+240)/2, (VIEW1_DLG1_HEIGHT+380)/2);
...
void CInspectViewExtra2Dlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CInspectViewExtra2Dlg)
	DDX_Control(pDX, IDC_VSCRBAR, m_VScrBar);
	DDX_Control(pDX, IDC_HSCRBAR, m_HScrBar);
	//}}AFX_DATA_MAP
}

BEGIN_MESSAGE_MAP(CInspectViewExtra2Dlg, CDialog)
	//{{AFX_MSG_MAP(CInspectViewExtra2Dlg)
	ON_WM_ERASEBKGND()
	ON_WM_PAINT()
	ON_WM_HSCROLL()
	ON_WM_VSCROLL()
...
/////////////////////////////////////////////////////////////////////////////
// CInspectViewExtra2Dlg message handlers

void CInspectViewExtra2Dlg::ClearAll()
{
	m_dZoomRatio = 0.4;
...
LRESULT CInspectViewExtra2Dlg::OnEventPcbModelChanaged(WPARAM wParam, LPARAM lParam)
{
	Reset();
...
// For Release
LRESULT CInspectViewExtra2Dlg::OnEventFrameInspectDone(WPARAM wParam, LPARAM lParam)
{
...
BOOL CInspectViewExtra2Dlg::PreTranslateMessage(MSG* pMsg) 
{
	switch (pMsg->message) {
...
int CInspectViewExtra2Dlg::Get_CurScanTab()
{
	return m_ViewToolbar.Get_CurComboBox();
}

Hlong CInspectViewExtra2Dlg::GetDispWindowID()
{
	return m_lWindowID;
}

void CInspectViewExtra2Dlg::Set_CurScanTab(int nScanNum)
{ 
	CString strScan;
...
void CInspectViewExtra2Dlg::Add_ScanTab(int nScanNum)
{
	CString strScan;
	strScan.Format("SCAN_%d", nScanNum);
...
void CInspectViewExtra2Dlg::Clear_ScanTab()
{
	m_ViewToolbar.Clear_ComboBox();
}

int CInspectViewExtra2Dlg::Get_ScanTabCount()
{
	return m_ViewToolbar.Get_ComboBoxCount();
}

int CInspectViewExtra2Dlg::Get_ScanTabIndex()
{
	return m_ViewToolbar.Get_ComboBoxIndex();
}

void CInspectViewExtra2Dlg::Set_ScanTabIndex(int nIdx)
{
	m_ViewToolbar.Set_ComboBoxIndex(nIdx);
}

BOOL CInspectViewExtra2Dlg::OnInitDialog() 
{
	CDialog::OnInitDialog();

	SetWindowText(g_sImageName[THEAPP.m_iMachineInspType][TRIGGER_EXTRA2]);
...
BOOL CInspectViewExtra2Dlg::OnEraseBkgnd(CDC *pDC)
{
 	pDC->FillSolidRect(m_ClientRect, TS_COLOR_EXTRA_BG);
	return TRUE;
}

void CInspectViewExtra2Dlg::OnPaint() 
{
...
void CInspectViewExtra2Dlg::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) 
{
...
void CInspectViewExtra2Dlg::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) 
{
	if (m_lWindowID < 0 || !m_bDisplayImage) return;
...
void CInspectViewExtra2Dlg::OnLButtonDown(UINT nFlags, CPoint point) 
{
...
void CInspectViewExtra2Dlg::OnLButtonUp(UINT nFlags, CPoint point) 
{
	if (!m_bDisplayImage) return;
...
void CInspectViewExtra2Dlg::OnMouseMove(UINT nFlags, CPoint point) 
{
	if (!m_bDisplayImage) return;
	...
	CString OutTxt;
	OutTxt.Format("%s - (%d, %d), Zoom(%d%%)", g_sImageName[THEAPP.m_iMachineInspType][TRIGGER_EXTRA2], (int)((point.x/m_dZoomRatio)+m_ViewRect.left), (int)((point.y/m_dZoomRatio)+m_ViewRect.top), (int)(m_dZoomRatio*100.));
...
void CInspectViewExtra2Dlg::OnDestroy() 
{
	CDialog::OnDestroy();
...
void CInspectViewExtra2Dlg::OnSize(UINT nType, int cx, int cy) 
{
...
BOOL CInspectViewExtra2Dlg::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt) 
{
...
BOOL CInspectViewExtra2Dlg::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message) 
{
	switch (m_ToolBarState) {
	case TS_MOVE:
		if (m_bOnMoving)
...
void CInspectViewExtra2Dlg::Reset()
{
...
void CInspectViewExtra2Dlg::InitViewRect() 
{
	m_ViewRect.left = 0;
	m_ViewRect.top = 0;
...
void CInspectViewExtra2Dlg::UpdateViewportManager()
{
...
void CInspectViewExtra2Dlg::ViewRectSet() 
{
...
void CInspectViewExtra2Dlg::ScrollBarSet()
{
	SCROLLINFO Info;
...
void CInspectViewExtra2Dlg::ScrollBarPosSet()
{
...
void CInspectViewExtra2Dlg::DrawActiveTRegion()
{
...
void CInspectViewExtra2Dlg::DrawSelectPartTRegion()
{
	CDC *pDC = GetDC();
...
void CInspectViewExtra2Dlg::ZoomInSelectedPart(double dZoomRatio, int iStartPosX, int iStartPosY) 
{
	if (m_bDisplayImage) {
...
//		m_ViewportManager.mdZoomRatio = m_dZoomRatio;
		...
		CString OutTxt;
		OutTxt.Format("%s - Zoom(%d%%)", g_sImageName[THEAPP.m_iMachineInspType][TRIGGER_EXTRA2], (int)(m_dZoomRatio * 100.));
...
void CInspectViewExtra2Dlg::OnTbDraw() 
{
	m_ToolBarState = TS_DRAW;
}

void CInspectViewExtra2Dlg::OnTbMove() 
{
 	m_ToolBarState = TS_MOVE;
}

void CInspectViewExtra2Dlg::OnTbSelectPart() 
{
 	m_ToolBarState = TS_SELECT_PART;
}

void CInspectViewExtra2Dlg::OnTbZoomIn() 
{
	if (m_bDisplayImage) {
...
void CInspectViewExtra2Dlg::OnTbZoomOut() 
{
	if (m_bDisplayImage) {
		m_dZoomRatio = (int)(m_dZoomRatio * 10) * 0.1;
...
void CInspectViewExtra2Dlg::OnTbFitWidth() 
{
	if (m_bDisplayImage) {
...
void CInspectViewExtra2Dlg::OnTbFitHeight() 
{
	if (m_bDisplayImage) {
...
BOOL CInspectViewExtra2Dlg::ShowWindow(int nCmdShow)
{
	return CDialog::ShowWindow(nCmdShow);
}

void CInspectViewExtra2Dlg::InitializeToolBar()
{
	CToolBarCtrl &toolbarctrl = m_ViewToolbar.GetToolBarCtrl();
...
int CInspectViewExtra2Dlg::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
...
LRESULT CInspectViewExtra2Dlg::On2DFrameReady(WPARAM wParam, LPARAM lParam)
{

	return 0;
}

LRESULT CInspectViewExtra2Dlg::OnEventGoToDefect(WPARAM wParam, LPARAM lParam)	
{
...
void CInspectViewExtra2Dlg::LoadScanImage(int nScanNum) 
{

}

void CInspectViewExtra2Dlg::MoveToDefectArea()
{
	
}

void CInspectViewExtra2Dlg::RedrawView()
{
	InvalidateRect(NULL, FALSE);
}

void CInspectViewExtra2Dlg::RedrawRect()
{
	InvalidateRect(&m_ClientRect, FALSE);
}

BOOL CInspectViewExtra2Dlg::SelectAllPackage()
{
	return FALSE;
}

BOOL CInspectViewExtra2Dlg::SelectPackageChange() 
{
	return FALSE;
}

BOOL CInspectViewExtra2Dlg::SelectPackageUndo() 
{
	return FALSE;
}

void CInspectViewExtra2Dlg::UnSelectAll()
{
