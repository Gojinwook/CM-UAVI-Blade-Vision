...
class CROICommonAlgorithmDlg : public CDialog
{
	DECLARE_DYNAMIC(CROICommonAlgorithmDlg)

public:
	CROICommonAlgorithmDlg(CWnd* pParent = NULL);   // 표준 생성자입니다.
	virtual ~CROICommonAlgorithmDlg();
	...
	// 대화 상자 데이터입니다.
	enum { IDD = IDD_TEACH_ROI_COMMON_ALGORITHM_DLG };

private:
	void	SetPosition(int left, int top, int right, int bottom) { m_ScreenRect = CRect(left, top, right, bottom); }
	CRect	m_ScreenRect;

	static CROICommonAlgorithmDlg* m_pInstance;
...
public:
	...
	BOOL m_bCheckUseCommonAlgorithm;
