// TrayOperatorViewPrevDlg.cpp : 구현 파일입니다.
//
...
CTrayOperatorViewPrevDlg* CTrayOperatorViewPrevDlg::m_pInstance = NULL;

CTrayOperatorViewPrevDlg* CTrayOperatorViewPrevDlg::GetInstance(BOOL bShowFlag)
{
	if (!m_pInstance) {
		m_pInstance = new CTrayOperatorViewPrevDlg();
		if(!m_pInstance->m_hWnd) {
			CMainFrame* pFrame = (CMainFrame*)AfxGetMainWnd();
			m_pInstance->Create(IDD_TRAYVIEW_OPERATOR_PREVIOUS_DLG, pFrame->GetActiveView());
			if (bShowFlag) m_pInstance->Show();
		}
	}
	return m_pInstance;
}
void CTrayOperatorViewPrevDlg::DeleteInstance()
{
...
void CTrayOperatorViewPrevDlg::Show()
{
	ShowWindow(SW_HIDE);
...
IMPLEMENT_DYNAMIC(CTrayOperatorViewPrevDlg, CDialog)

CTrayOperatorViewPrevDlg::CTrayOperatorViewPrevDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CTrayOperatorViewPrevDlg::IDD, pParent)
{
		SetPosition(VIEW1_DLG1_LEFT, VIEW1_DLG1_TOP, VIEW1_DLG1_WIDTH+20, VIEW1_DLG1_HEIGHT+380);
}

CTrayOperatorViewPrevDlg::~CTrayOperatorViewPrevDlg()
{
}

void CTrayOperatorViewPrevDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CTrayOperatorViewPrevDlg, CDialog)
	ON_WM_CTLCOLOR()
END_MESSAGE_MAP()


// CTrayOperatorViewPrevDlg 메시지 처리기입니다.


BOOL CTrayOperatorViewPrevDlg::PreTranslateMessage(MSG* pMsg)
{
...
HBRUSH CTrayOperatorViewPrevDlg::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
CFont* CTrayOperatorViewPrevDlg::GetFont(LONG lfWidth, LONG lfHeight, LONG lfWeight)
 {
        CFont* pFont;
        LOGFONT logFont;
...
void CTrayOperatorViewPrevDlg::MakeTrayRectangle()
{
	...
	CString temp;
	temp.Format("CTrayOperatorViewPrevDlg makMakeTrayRectangle %d %d", THEAPP.m_pModelDataManager->m_iTrayArrayX, THEAPP.m_pModelDataManager->m_iTrayArrayY);
	THEAPP.SaveLog(temp);
