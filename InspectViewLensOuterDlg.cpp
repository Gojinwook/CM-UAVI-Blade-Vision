...
/////////////////////////////////////////////////////////////////////////////
// CInspectViewLensOuterDlg dialog

CInspectViewLensOuterDlg* CInspectViewLensOuterDlg::m_pInstance = NULL;

CInspectViewLensOuterDlg* CInspectViewLensOuterDlg::GetInstance(BOOL bShowFlag)
{
	if (!m_pInstance) {
		m_pInstance = new CInspectViewLensOuterDlg();
		if(!m_pInstance->m_hWnd) {
			CMainFrame* pFrame = (CMainFrame*)AfxGetMainWnd();
			m_pInstance->Create(IDD_INSPECT_VIEW_LENS_OUTER_DLG, pFrame->GetActiveView());
...
void CInspectViewLensOuterDlg::DeleteInstance()
{
...
void CInspectViewLensOuterDlg::Show()
{
	ShowWindow(SW_HIDE);
...
CInspectViewLensOuterDlg::CInspectViewLensOuterDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CInspectViewLensOuterDlg::IDD, pParent)
{
	//{{AFX_DATA_INIT(CInspectViewLensOuterDlg)
		// NOTE: the ClassWizard will add member initialization here
	//}}AFX_DATA_INIT
	SetPosition(VIEW1_DLG1_LEFT+(VIEW1_DLG1_WIDTH+270)/2, VIEW1_DLG1_TOP+(VIEW1_DLG1_HEIGHT+380)/2, (VIEW1_DLG1_WIDTH+290)/2, (VIEW1_DLG1_HEIGHT+380)/2);
...
void CInspectViewLensOuterDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CInspectViewLensOuterDlg)
	DDX_Control(pDX, IDC_VSCRBAR, m_VScrBar);
	DDX_Control(pDX, IDC_HSCRBAR, m_HScrBar);
	//}}AFX_DATA_MAP
}

BEGIN_MESSAGE_MAP(CInspectViewLensOuterDlg, CDialog)
	//{{AFX_MSG_MAP(CInspectViewLensOuterDlg)
	ON_WM_ERASEBKGND()
	ON_WM_PAINT()
	ON_WM_HSCROLL()
	ON_WM_VSCROLL()
...
/////////////////////////////////////////////////////////////////////////////
// CInspectViewLensOuterDlg message handlers

void CInspectViewLensOuterDlg::ClearAll()
{
	m_dZoomRatio = 0.4;
...
LRESULT CInspectViewLensOuterDlg::OnEventPcbModelChanaged(WPARAM wParam, LPARAM lParam)
{
	Reset();
...
// For Release
LRESULT CInspectViewLensOuterDlg::OnEventFrameInspectDone(WPARAM wParam, LPARAM lParam)
{
...
BOOL CInspectViewLensOuterDlg::PreTranslateMessage(MSG* pMsg) 
{
	switch (pMsg->message) {
...
int CInspectViewLensOuterDlg::Get_CurScanTab()
{
	return m_ViewToolbar.Get_CurComboBox();
}

void CInspectViewLensOuterDlg::Set_CurScanTab(int nScanNum)
{ 
	CString strScan;
...
void CInspectViewLensOuterDlg::Add_ScanTab(int nScanNum)
{
	CString strScan;
	strScan.Format("SCAN_%d", nScanNum);
...
void CInspectViewLensOuterDlg::Clear_ScanTab()
{
	m_ViewToolbar.Clear_ComboBox();
}

int CInspectViewLensOuterDlg::Get_ScanTabCount()
{
	return m_ViewToolbar.Get_ComboBoxCount();
}

int CInspectViewLensOuterDlg::Get_ScanTabIndex()
{
	return m_ViewToolbar.Get_ComboBoxIndex();
}

void CInspectViewLensOuterDlg::Set_ScanTabIndex(int nIdx)
{
	m_ViewToolbar.Set_ComboBoxIndex(nIdx);
}

BOOL CInspectViewLensOuterDlg::OnInitDialog() 
{
...
BOOL CInspectViewLensOuterDlg::OnEraseBkgnd(CDC *pDC)
{
 	pDC->FillSolidRect(m_ClientRect, TS_COLOR_EXTRA_BG);
	return TRUE;
}

void CInspectViewLensOuterDlg::OnPaint() 
{
...
void CInspectViewLensOuterDlg::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) 
{
...
void CInspectViewLensOuterDlg::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) 
{
	if (m_lWindowID < 0 || !m_bDisplayImage) return;
...
void CInspectViewLensOuterDlg::OnLButtonDown(UINT nFlags, CPoint point) 
{
...
void CInspectViewLensOuterDlg::OnLButtonUp(UINT nFlags, CPoint point) 
{
	if (!m_bDisplayImage) return;
...
void CInspectViewLensOuterDlg::OnMouseMove(UINT nFlags, CPoint point) 
{
	if (!m_bDisplayImage) return;
	...
	CString OutTxt;
	OutTxt.Format("Lens Outer - (%d, %d), Zoom(%d%%)", (int)((point.x/m_dZoomRatio)+m_ViewRect.left), (int)((point.y/m_dZoomRatio)+m_ViewRect.top), (int)(m_dZoomRatio*100.));
...
void CInspectViewLensOuterDlg::OnDestroy() 
{
	CDialog::OnDestroy();
...
void CInspectViewLensOuterDlg::OnSize(UINT nType, int cx, int cy) 
{
...
BOOL CInspectViewLensOuterDlg::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt) 
{
...
BOOL CInspectViewLensOuterDlg::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message) 
{
	switch (m_ToolBarState) {
	case TS_MOVE:
		if (m_bOnMoving)
...
void CInspectViewLensOuterDlg::Reset()
{
...
void CInspectViewLensOuterDlg::InitViewRect() 
{
	m_ViewRect.left = 0;
	m_ViewRect.top = 0;
...
void CInspectViewLensOuterDlg::UpdateViewportManager()
{
...
void CInspectViewLensOuterDlg::ViewRectSet() 
{
...
void CInspectViewLensOuterDlg::ScrollBarSet()
{
	SCROLLINFO Info;
...
void CInspectViewLensOuterDlg::ScrollBarPosSet()
{
...
void CInspectViewLensOuterDlg::DrawActiveTRegion()
{
...
void CInspectViewLensOuterDlg::DrawSelectPartTRegion()
{
	CDC *pDC = GetDC();
...
void CInspectViewLensOuterDlg::ZoomInSelectedPart(double dZoomRatio, int iStartPosX, int iStartPosY) 
{
	if (m_bDisplayImage) {
...
//		m_ViewportManager.mdZoomRatio = m_dZoomRatio;
		...
		CString OutTxt;
		OutTxt.Format("Lens Outer - Zoom(%d%%)", (int)(m_dZoomRatio * 100.));
...
void CInspectViewLensOuterDlg::OnTbDraw() 
{
	m_ToolBarState = TS_DRAW;
}

void CInspectViewLensOuterDlg::OnTbMove() 
{
 	m_ToolBarState = TS_MOVE;
}

void CInspectViewLensOuterDlg::OnTbSelectPart() 
{
 	m_ToolBarState = TS_SELECT_PART;
}

void CInspectViewLensOuterDlg::OnTbZoomIn() 
{
	if (m_bDisplayImage) {
...
void CInspectViewLensOuterDlg::OnTbZoomOut() 
{
	if (m_bDisplayImage) {
		m_dZoomRatio = (int)(m_dZoomRatio * 10) * 0.1;
...
void CInspectViewLensOuterDlg::OnTbFitWidth() 
{
	if (m_bDisplayImage) {
...
void CInspectViewLensOuterDlg::OnTbFitHeight() 
{
	if (m_bDisplayImage) {
...
BOOL CInspectViewLensOuterDlg::ShowWindow(int nCmdShow)
{
	return CDialog::ShowWindow(nCmdShow);
}

void CInspectViewLensOuterDlg::InitializeToolBar()
{
	CToolBarCtrl &toolbarctrl = m_ViewToolbar.GetToolBarCtrl();
...
int CInspectViewLensOuterDlg::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
...
LRESULT CInspectViewLensOuterDlg::On2DFrameReady(WPARAM wParam, LPARAM lParam)
{

	return 0;
}

LRESULT CInspectViewLensOuterDlg::OnEventGoToDefect(WPARAM wParam, LPARAM lParam)	
{
...
void CInspectViewLensOuterDlg::LoadScanImage(int nScanNum) 
{

}

void CInspectViewLensOuterDlg::MoveToDefectArea()
{
	
}

void CInspectViewLensOuterDlg::RedrawView()
{
	InvalidateRect(NULL, FALSE);
}

void CInspectViewLensOuterDlg::RedrawRect()
{
	InvalidateRect(&m_ClientRect, FALSE);
}

BOOL CInspectViewLensOuterDlg::SelectAllPackage()
{
	return FALSE;
}

BOOL CInspectViewLensOuterDlg::SelectPackageChange() 
{
	return FALSE;
}

BOOL CInspectViewLensOuterDlg::SelectPackageUndo() 
{
	return FALSE;
}

void CInspectViewLensOuterDlg::UnSelectAll()
{
